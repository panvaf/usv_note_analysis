...
def get_spec(t1, t2, audio, p, fs=32000, target_freqs=None, target_times=None, \
	...
	* Why is a flag returned?
	...
	if temp < p['nperseg'] or s2 <= 0 or s1 >= len(audio):
		...
		if p['notch_filter']:
			nq_freq = 0.5*p['fs']
			notch_freq = p['fn'] / nq_freq
			b,a = iirnotch(notch_freq,p['q'])
			temp_audio = lfilter(b,a,temp_audio)
	...
	spec = interp_spec
	# Remove noise
	loc = int((p['fn']-p['min_freq'])/(p['max_freq']-p['min_freq'])*p['num_freq_bins'])
	spec[loc,:] = 0
	...
	# Filter
	spec = gaussian_filter(spec,p['sigma'])
	# Within-syllable normalize.
	if p['within_syll_normalize'] and max_spec>p['spec_noise_thres']:
		spec -= np.quantile(spec, p['normalize_quantile'])
		spec[spec<0.0] = 0.0
		spec /= np.max(spec) + EPSILON
	# Flip spectogram for control experiments
	if p['flip']:
		spec = np.flipud(spec)
	return spec, True
